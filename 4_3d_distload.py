...
import os
from eurocode_formulas import M_c_Rd
...
surf2 = ax1.plot_surface(V_mesh, H_mesh, M_Th2_mesh, cmap='viridis', 
                         edgecolor='none', alpha=0.7, label='Th. 2')

ax1.set_xlabel('Vertical Force v [kN/m]')
ax1.set_ylabel('Horizontal Force h [kN/m]')
ax1.set_zlabel('Internal Moment M [kNm]')
ax1.set_title('M as function of V and H', fontsize=12, fontweight='bold')
...
ax2.set_xlabel('Vertical Force v [kN/m]')
ax2.set_ylabel('Horizontal Force h [kN/m]')
ax2.set_title('Internal Moment M - Contour Plot', fontsize=12, fontweight='bold')
fig1.colorbar(contour, ax=ax2, label='M [kNm]')
...
for i in range(0, len(H_range), 4): # Plot every 4th H-level for clarity
    ax3.plot(V_range, M_Th2_mesh[i, :], label=f'H = {H_range[i]:.1f} kN')

ax3.set_xlabel('Vertical Force v [kN/m]')
ax3.set_ylabel('Internal Moment M [kNm]')
ax3.set_title('M as function of v (Various h)', fontweight='bold')
...
for i in range(0, len(V_range), 4): # Plot every 4th V-level
    ax4.plot(H_range, M_Th2_mesh[:, i], label=f'V = {V_range[i]:.1f} kN')

ax4.set_xlabel('Horizontal Force h [kN/m]')
ax4.set_ylabel('Internal Moment M [kNm]')
ax4.set_title('M as function of h (Various v)', fontweight='bold')
