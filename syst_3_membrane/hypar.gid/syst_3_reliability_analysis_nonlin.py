...
# ---------------------------------------------------------------------------------------
# Evaluating the points for defining t_S_hyperplane_linearized

# t_S_l1k_0 = t_S_hypar_nonlinear(l_1k,0)
# t_S_0_l2k = t_S_hypar_nonlinear(0,l_2k)
# t_S_0_0 = t_S_hypar_nonlinear(0,0)


# print(f"t_S_l1k_0 = {t_S_l1k_0}")
# print(f"t_S_0_l2k = {t_S_0_l2k}")
# print(f"t_S_0_0 = {t_S_0_0}")
...
# Design Opt 1
e_d_1 = t_S_hypar_nonlinear(l_1d, l_2d)

# Design Opt 2
argument_1 = gamma_F1 * t_S_hypar_nonlinear(l_1k,  (gamma_F2 / gamma_F1) * l_2k)
argument_2 = gamma_F2 * t_S_hypar_nonlinear((gamma_F1 / gamma_F2) * l_1k, l_2k)
e_d_2 = max(argument_1, argument_2)

# Design Opt 2'
argument_1_primed = gamma_F1 * (t_S_hypar_nonlinear(l_1k,  (gamma_F2 / gamma_F1) * l_2k) - t_S_hypar_nonlinear(0,0)) + t_S_hypar_nonlinear(0,0)
argument_2_primed = gamma_F2 * (t_S_hypar_nonlinear((gamma_F1 / gamma_F2) * l_1k, l_2k) - t_S_hypar_nonlinear(0,0)) + t_S_hypar_nonlinear(0,0)
...
def g_opt1_FORM(x):
    ...
    L1_eff = np.clip(np.nan_to_num(L1_raw, nan=0.0, posinf=60, neginf=0.0), 0.0, 60)
    L2_eff = np.clip(np.nan_to_num(L2_raw, nan=0.0, posinf=60, neginf=0.0), 0.0, 60)
...
def g_opt1_FORM(x):
    ...
    action_side = t_S_hypar_nonlinear(L1_eff, L2_eff)
