...
Fz_range = np.linspace(0, l_1d * 1.1, 15) # Load in kN/m2
Fy_range = np.linspace(0, l_2d * 1.1, 15) # Load in kN/m2
...
for i in range(len(Fy_range)):
        for j in range(len(Fz_range)):
            ...
            sigma = t_S_hypar_nonlinear(Fz_val, Fy_val)
...
t_S_l1d_0 = t_S_hypar_nonlinear(l_1d, 0)
t_S_l1k_0 = t_S_hypar_nonlinear(l_1d / 1.5, 0)
t_S_0_l2d = t_S_hypar_nonlinear(0, l_2d)
t_S_0_l2k = t_S_hypar_nonlinear(0, l_2d / 1.5)
